...
export function handleKeyboard(e) {
    ...
    if (e.ctrlKey) {
        if (e.key === 'ArrowRight') {
            changed = gantt.shift.shiftItem(state.projectData.roots, state.selectedTaskFullId, 'right');
        } else if (e.key === 'ArrowLeft') {
            changed = gantt.shift.shiftItem(state.projectData.roots, state.selectedTaskFullId, 'left');
        } else if (e.key === 'ArrowUp') {
            changed = gantt.up.moveUp(state.projectData.roots, state.selectedTaskFullId);
        } else if (e.key === 'ArrowDown') {
            changed = gantt.down.moveDown(state.projectData.roots, state.selectedTaskFullId);
        }
    }
