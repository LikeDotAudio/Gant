...
export function renderNav(canvas, viewport, container, chart, flatTasks, minStr, maxStr, zoomLevel, milestones) {
    ...
    // Draw Milestones in the navigator
    if (milestones) {
        ...
        milestones.forEach(m => {
            ...
            const x = (stickyWidthPx + mX) * scaleX;
            ctx.beginPath();
            ctx.moveTo(x, 0);
            ctx.lineTo(x, ch);
            ctx.stroke();
